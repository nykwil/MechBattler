# Archived — the Jul 2026 status record

This is the historical body of `07-status-and-handoff.md` as it stood before the
Aug 2026 audit: the combat-skeleton simplifications, the archetype/RPS work, the
keystone-fitting split, tuning pass 1, the balance-harness standings as they moved
week by week, and the two Build Week passes.

It is kept because several sections are records of things that were *tried*, with
the reasoning, and because the standings tables are the only written trace of how
the roster moved. **None of the numbers in it are current** — they predate the
progression loop (`docs/16`), the spatial/height work, and the mobile port. For
what is true now, read `docs/07-status-and-handoff.md`; for balance causes, read
...
`docs/archive/submission/TUNING-REPORT.md`.


## Roadmap as written Jul 17 2026 (superseded)
