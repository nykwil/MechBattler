...
export function auditPartDifferentiation(): PartDifferentiationFinding[] {
  return [
    ...
    { parts: 'U-PLINTH vs U-RISE2 vs U-RISEL vs U-TUR', verdict: 'distinct', evidence: 'Risers are not a cost ladder, they are a shape family: a stacked part must cover exactly the cells of the part beneath it, so each riser raises the guns whose footprint it matches and is dead weight under everything else. The family covered 14 of 17 weapons and missed exactly three -- W-RG, W-SR and W-PIN -- two of which are the guns every sweep reports as unreachable. The Plinth is the 1-cell member: the Pin, and the single spare cell a light frame is always left with. It is also the first content on electricalCapacityKw, which is live (spatialPower.ts widest-path, read twice by simulation.ts) but flat -- ten of eleven declarations are the 60 kW wire default. At 20 kW it throttles any power PATH routed through it -- measured, a laser fed through a Plinth resolves at 20 kW and lands in bottleneckInstanceIds, where the same laser through a coupler resolves at 60 -- so the cheapest riser in the catalog is also a pinch point, and a build that leans on it for structure pays for that in its wiring rather than in its cells. docs/17 F75.' },
    ...
    { parts: 'U-TUR vs U-RISE2', verdict: 'overlap-watch', evidence: 'Corrected 3 Sep 2026 (docs/17 F75). Arc is still inert -- the pilot holds its target to a mean 0.08 degrees and the arc gate fires in 0.00% of weapon-frames -- so the +25 degrees is never collected. But "a strictly more expensive Block" was wrong, and badly: a riser must match its payload footprint EXACTLY (checkStackLegality refuses a partial cover as footprint-mismatch), so these two are not substitutes at all. The Gimbal is 2 cells and raises the 2-cell guns (Stitcher, Needle); the Block is 4 and raises the 4-cell ones (Pepperbox, Anvil, Lance, Scald). Fitting the wrong one is not a small overpayment: on a 16-cell Vulture carrying carbines, adding a Block costs 40 win points (94.3% to 53.7%, net -71 of 175 paired fights, z=-7.6) while adding a Gimbal costs nothing measurable. The Gimbal also conducts at 1.5 against the default 1. What it overpays for is the arc, not the cells.' },
